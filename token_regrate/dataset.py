...
def _normalize_train_data_sources(sources):
    """Normalize None, a single path/URL string, or an iterable into a source list."""
    if sources is None:
        return []
    if isinstance(sources, (str, os.PathLike)):
        item = os.fspath(sources).strip()
...
def _split_train_data_sources(sources):
    ...
    cc12m_tsv = None
    hf_urls = []
    for source in _normalize_train_data_sources(sources):
